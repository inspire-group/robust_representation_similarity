...
class SnowAttack:
    ...
    def _init(self, batch_size):
        ...
        flake_intensities = torch.exp(
            -1.0
            / (self.budget)
            * torch.rand(batch_size, 7, self.resol // 4, self.resol // 4)
        ).cuda()
    ...
    def _forward(
        ...
        s = pixel_model(apply_snow(pixel_inp / 255.0, snow, base_eps) * 255)
        ...
        for it in range(self.nb_its):
            ...
            flake_intensities.data = (
                flake_intensities.data + step_size[:, None, None, None] * grad_sign
            )
            ...
            if it != self.nb_its - 1:
                ...
                for i in range(blocks):
                    for j in range(blocks):
                        ...
                        ] *= self.budget / budget_per_region[:, :, i, j].view(
                            -1, 7, 1, 1
                        )
        ...
        snow = snow_creator(flake_intensities, kernels, self.resol)
        pixel_result = apply_snow(pixel_inp / 255.0, snow, base_eps) * 255
