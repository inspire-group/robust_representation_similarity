# Borrowed from https://github.com/ddkang/advex-uar
import numpy as np
import torch
...
def rgb_to_ycbcr(image):
    ...
    shift = torch.as_tensor([16.0, 128.0, 128.0], device="cuda")
...
def rgb_to_ycbcr_jpeg(image):
    ...
    shift = torch.as_tensor([0.0, 128.0, 128.0], device="cuda")

    # result = torch.tensordot(image, torch.as_tensor(matrix, device="cuda"), dims=1) + shift
    result = (
        tensordot_pytorch(image, torch.as_tensor(matrix, device="cuda"), dims=1) + shift
    )
    result.view(image.size())
    return result
...
def dct_8x8(image):
    ...
    result = scale * tensordot_pytorch(
        image, torch.as_tensor(tensor, device="cuda"), dims=2
    )
    result.view(image.size())
    return result
...
def idct_8x8(image):
    ...
    result = (
        0.25 * tensordot_pytorch(image, torch.as_tensor(tensor, device="cuda"), dims=2)
        + 128
    )
    result.view(image.size())
    return result
...
def ycbcr_to_rgb(image):
    ...
    result = (
        tensordot_pytorch(image, torch.tensor(matrix, device="cuda"), dims=1) + shift
    )
    result.view(image.size())
    return result
...
def ycbcr_to_rgb_jpeg(image):
    matrix = np.array(
        [[1.0, 0.0, 1.402], [1, -0.344136, -0.714136], [1, 1.772, 0]], dtype=np.float32
    ).T
    shift = torch.FloatTensor([0, -128, -128]).cuda()

    # result = torch.tensordot(image + shift, torch.tensor(matrix, device="cuda"), dims=1)
    result = tensordot_pytorch(
        image + shift, torch.tensor(matrix, device="cuda"), dims=1
    )
    result.view(image.size())
    return result


def jpeg_compress_decode(
    self, image_channels_first, rounding_vars, lambder, downsample_c=True, factor=1
):
    ...
    for k in components.keys():
        ...
        if k == "y":
            comp = y_quantize(
                self, comp, noisy_round, 0.5 + 0.5 * rounding_vars[0], factor
            )
        elif k == "cb":
            comp = c_quantize(
                self, comp, noisy_round, 0.5 + 0.5 * rounding_vars[1], factor
            )
        else:
            comp = c_quantize(
                self, comp, noisy_round, 0.5 + 0.5 * rounding_vars[2], factor
            )
        components[k] = comp
    ...
    for k in components.keys():
        comp = components[k]
        comp = (
            c_dequantize(self, comp, factor)
            if k in ("cb", "cr")
            else y_dequantize(self, comp, factor)
        )
        comp = idct_8x8(comp)
        if k in ("cb", "cr"):
            if downsample_c:
...
class JPEGAttack:
    ...
    def _convert_cat_var(self, cat_var, batch_size, height, width):
        y_var = cat_var[:, : height // 8 * width // 8 * 8 * 8].view(
            (batch_size, height // 8 * width // 8, 8, 8)
        )
        cb_var = cat_var[
            :,
            height // 8 * width // 8 * 8 * 8 : height // 8 * width // 8 * 8 * 8
            + height // 16 * width // 16 * 8 * 8,
        ].view((batch_size, height // 16 * width // 16, 8, 8))
        cr_var = cat_var[
            :,
            height // 8 * width // 8 * 8 * 8
            + height // 16 * width // 16 * 8 * 8 : height // 8 * width // 8 * 8 * 8
            + 2 * height // 16 * width // 16 * 8 * 8,
        ].view((batch_size, height // 16 * width // 16, 8, 8))
        return y_var, cb_var, cr_var

    def _jpeg_cat(self, pixel_inp, cat_var, base_eps, batch_size, height, width):
        y_var, cb_var, cr_var = self._convert_cat_var(
            cat_var, batch_size, height, width
        )
    ...
    def _run_one_pgd(
        ...
        pixel_inp_jpeg = self._jpeg_cat(
            pixel_inp, cat_var, base_eps, batch_size, height, width
        )
        ...
        for it in range(self.nb_its):
            ...
            if self.opt == "linf":
                ...
                cat_var.data = torch.max(
                    torch.min(cat_var.data, base_eps[:, None]), -base_eps[:, None]
                )  # cannot use torch.clamp for tensors
            elif self.opt == "l2":
                ...
                proj_scale = torch.min(
                    torch.ones_like(l2_delta, device="cuda"), base_eps / l2_delta
                )
            ...
            if it != self.nb_its - 1:
                ...
                pixel_inp_jpeg = self._jpeg_cat(
                    pixel_inp, cat_var_temp, base_eps, batch_size, height, width
                )
                s = pixel_model(pixel_inp_jpeg)
    ...
    def _run_one_fw(
        self, pixel_model, pixel_inp, cat_var, target, base_eps, avoid_target=True
    ):
        batch_size, channels, height, width = pixel_inp.size()
        pixel_inp_jpeg = self._jpeg_cat(
            pixel_inp, cat_var, base_eps, batch_size, height, width
        ...
        for it in range(self.nb_its):
            ...
            bound = where_float(
                sign_grad > 0, self.l1_max - cat_var, cat_var + self.l1_max
            ).view(batch_size, -1)

            k_min = torch.zeros(
                (batch_size, 1), dtype=torch.long, requires_grad=False, device="cuda"
            )
            k_max = (
                torch.ones(
                    (batch_size, 1),
            ...
            magnitudes = torch.zeros(
                (batch_size, num_pixels), requires_grad=False, device="cuda"
            )
            for bi in range(batch_size):
                magnitudes[bi, indices[bi, : k_min[bi, 0]]] = bnd[bi, : k_min[bi, 0]]
                magnitudes[bi, indices[bi, k_min[bi, 0]]] = (
                    base_eps[bi] - cum_bnd[bi, k_min[bi, 0]]
                )
    ...
    def _init_l2(self, batch_size, height, width):
        ...
        rand_norms = torch.pow(
            torch.rand(init.size()[0], dtype=torch.float32, device="cuda"), 1 / shape[1]
        )
    ...
    def _forward(
        self, pixel_model, pixel_img, target, scale_eps=False, avoid_target=False
    ):
