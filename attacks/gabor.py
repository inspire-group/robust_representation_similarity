...
def gabor_rand_distributed(sp_conv, gabor_kernel):
    ...
    sp_conv = F.conv2d(
        sp_conv, weight=gabor_kernel, stride=1, groups=batch_size, padding=11
    )
...
class GaborAttack:
    def __init__(
        self, nb_its, eps_max, step_size, resol, rand_init=True, scale_each=False
    ):
        """
        Parameters:
    ...
    def _init(self, batch_size, num_kern):
        ...
        if self.rand_init:
            ...
            sp_conv_numpy = sparse.random(
                self.resol * batch_size, self.resol, density=1.0 / grid, format="csr"
            )
    ...
    def _get_gabor_kernel(self, batch_size):
        ...
        for b in range(batch_size):
            ...
            kernels.append(
                get_gabor_with_sides(k_size, sigma, Lambda, theta, sides).cuda()
            )
        gabor_kernel = torch.cat(kernels, 0).view(-1, 1, k_size, k_size)
        return gabor_kernel

    def _forward(
        self, pixel_model, pixel_img, target, avoid_target=True, scale_eps=False
    ):
        pixel_inp = (
            pixel_img.detach() * 255.0
        )  # Assuming that input pixel are in [0., 1.]
        ...
        s = pixel_model(
            torch.clamp(
                pixel_inp + base_eps[:, None, None, None] * gabor_noise, 0.0, 255.0
            )
        )
        ...
        for it in range(self.nb_its):
            ...
            grad_sign = grad.sign()
            gabor_vars.data = (
                gabor_vars.data + step_size[:, None, None, None] * grad_sign
            )
            gabor_vars.data = torch.clamp(gabor_vars.data, -1, 1) * mask

            if it != self.nb_its - 1:
                gabor_noise = gabor_rand_distributed(gabor_vars, gabor_kernel).expand(
                    -1, 3, -1, -1
                )
                s = pixel_model(
                    torch.clamp(
                        pixel_inp + base_eps[:, None, None, None] * gabor_noise,
                        0.0,
                        255.0,
                    )
                )
                gabor_vars.grad.data.zero_()
        pixel_result = torch.clamp(
            pixel_inp
            + base_eps[:, None, None, None]
            * gabor_rand_distributed(gabor_vars, gabor_kernel),
            0.0,
            255.0,
        )
