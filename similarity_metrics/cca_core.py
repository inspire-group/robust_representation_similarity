...
# TODO: Stip away unnecessary modules from these files.
...
def compute_ccas(sigma_xx,
    ...
    (sigma_xx, sigma_xy, sigma_yx, sigma_yy, x_idxs,
     y_idxs) = remove_small(sigma_xx, sigma_xy, sigma_yx, sigma_yy, epsilon)
    ...
    inv_xx = np.linalg.pinv(sigma_xx, hermitian=True)
    inv_yy = np.linalg.pinv(sigma_yy, hermitian=True)
...
def robust_cca_similarity(acts1,
                          acts2,
                          threshold=0.98,
                          epsilon=1e-6,
                          compute_dirns=True):
